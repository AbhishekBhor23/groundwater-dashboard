...
def load_metadata(filepath):
    ...
    try:
        ...
        st.error(f"Metadata file not found at the specified path: {filepath}")
        return None
    except Exception as e:
...
if metadata_df is not None:
    for col in metadata_df.select_dtypes(['object']):
    ...
    villages = sorted(metadata_df[(metadata_df['State'] == selected_state) & (metadata_df['District'] == selected_district) & (metadata_df['Block'] == selected_block)]['Village'].dropna().unique())
    selected_village = st.sidebar.selectbox("Select Village", villages)
    wells = sorted(metadata_df[(metadata_df['State'] == selected_state) & (metadata_df['District'] == selected_district) & (metadata_df['Block'] == selected_block) & (metadata_df['Village'] == selected_village)]['WellNo'].dropna().unique())
    selected_well = st.sidebar.selectbox("Select Well Number", wells)
    if st.sidebar.button("Find Well"):
        st.session_state['well_no'] = selected_well
        st.query_params["wellNo"] = selected_well
        st.rerun()

# --- URL and Session State Handling for Well Number ---
...
if current_well_no:
    with st.spinner(f"Loading data for well: {current_well_no}..."):
        ...
        if df is not None:
            ...
            st.error(f"Failed to load data for well: {current_well_no}.")
